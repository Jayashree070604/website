...
public class SoftwaresController : Controller
{
    ...
    public SoftwaresController(ILinkService linkService, IActivityLogService activityLogService)
    {
    ...
    public async Task<IActionResult> Index(string? searchText, int? projectId, int? categoryId)
    {
        ...
        var softwareLinks = links.Where(x => IsExeUrl(x.Url)).ToList();
        ...
        var viewModel = new LinkListViewModel
        {
            ...
            Links = softwareLinks.Select(x => new LinkListItemViewModel
            {
                Id = x.Id,
                Title = x.Title,
                Url = x.Url,
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        if (link is null || !IsExeUrl(link.Url))
        {
            return NotFound();
        }
    ...
    public async Task<IActionResult> Download(int id)
    {
        ...
        {
            return BadRequest("Invalid software URL.");
        }

        await LogActivityAsync("Software Downloaded", $"Software '{link.Title}' download link opened.", link.ProjectId, "Software", link.Id.ToString());
    ...
    public async Task<IActionResult> Create(LinkUpsertViewModel viewModel)
    {
        ...
        if (!IsExeUrl(viewModel.Url))
        {
            ModelState.AddModelError(nameof(viewModel.Url), "Software URL must point to a .exe file.");
            viewModel.ProjectOptions = await BuildProjectOptionsAsync();
            viewModel.CategoryOptions = await BuildCategoryOptionsAsync();
            return View(viewModel);
        }
        ...
        if (!result.Success)
        {
            ModelState.AddModelError(string.Empty, result.Error ?? "Unable to create software link.");
            viewModel.ProjectOptions = await BuildProjectOptionsAsync();
            viewModel.CategoryOptions = await BuildCategoryOptionsAsync();
            return View(viewModel);
        }

        await LogActivityAsync("Software Link Created", $"Software link '{link.Title}' created.", link.ProjectId, "Software", link.Id.ToString());

        TempData["SuccessMessage"] = "Software link created successfully.";
    ...
    public async Task<IActionResult> Edit(LinkUpsertViewModel viewModel)
    {
        ...
        if (!result.Success)
        {
            ModelState.AddModelError(string.Empty, result.Error ?? "Unable to update software link.");
            viewModel.ProjectOptions = await BuildProjectOptionsAsync();
            viewModel.CategoryOptions = await BuildCategoryOptionsAsync();
            return View(viewModel);
        }

        await LogActivityAsync("Software Link Updated", $"Software link '{link.Title}' updated.", link.ProjectId, "Software", link.Id.ToString());

        TempData["SuccessMessage"] = "Software link updated successfully.";
        return RedirectToAction(nameof(Index));
    }
    ...
    public async Task<IActionResult> DeleteConfirmed(int id)
    {
        ...
        await LogActivityAsync("Software Link Deleted", $"Software link with id '{id}' deleted.", entityType: "Software", entityId: id.ToString());

        TempData["SuccessMessage"] = "Software link deleted successfully.";
        return RedirectToAction(nameof(Index));
    }
    ...
    private static bool IsExeUrl(string? url)
    {
